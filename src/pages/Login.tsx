...
function saveAttempts(data: AttemptData) { sessionStorage.setItem(STORAGE_KEY, JSON.stringify(data)) }
function clearAttempts() { sessionStorage.removeItem(STORAGE_KEY) }
...
export default function Login() {
  ...
  useEffect(() => {
    ...
    const timer = setInterval(() => {
      ...
      if (rem <= 0) { setLockoutRemaining(0); clearInterval(timer) }
  ...
  function friendlyError(msg: string): string {
    ...
    if (m.includes('too many requests') || m.includes('rate limit')) return 'Too many attempts. Please wait.'
    return msg
  }
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    if (Date.now() < attempts.lockedUntil) {
      setError(`Too many failed attempts. Try again in ${Math.ceil((attempts.lockedUntil - Date.now()) / 1000)}s.`)
      return
    }
    setLoading(true)
    if (mode === 'login') {
      ...
      if (!loginErr) { clearAttempts(); navigate('/'); return }
        ...
        <div className="rounded-2xl border p-8" style={{ background: config.card, borderColor: config.border }}>
          <div className="flex mb-6 rounded-lg overflow-hidden border" style={{ borderColor: config.border }}>
            {(['login', 'signup'] as const).map(tab => (
              <button key={tab} onClick={() => { setMode(tab); setError(''); setSuccessMsg('') }}
                className="flex-1 py-2.5 text-sm font-semibold transition-all"
                style={{ background: mode === tab ? config.primary : 'transparent', color: mode === tab ? '#000' : config.muted }}>
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            {mode === 'signup' && (
              <div>
                ...
                <input type="text" value={username} onChange={e => setUsername(e.target.value)}
                  placeholder="sqlmaster" required disabled={isLocked}
            ...
            <div>
              ...
              <input type="password" value={password} onChange={e => setPassword(e.target.value)}
                placeholder="••••••••" required minLength={6} disabled={isLocked}
                className="w-full px-4 py-3 rounded-xl border outline-none disabled:opacity-50"
                style={{ background: config.bg, borderColor: config.border, color: config.text }} />
            </div>
            {error && !isLocked && (
